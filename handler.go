...
func PostBalasan(w http.ResponseWriter, r *http.Request) {
	...
	link := "https://medium.com/@hilaldzakwan08/apa-itu-gempa-bumi-add250ae5cfb"
	if r.Header.Get("Secret") == os.Getenv("SECRET") {
		if msg.Message == "loc" || msg.Message == "Loc" || msg.Message == "lokasi" || msg.LiveLoc {
			...
			reply := fmt.Sprintf("Konnichiwa kamu pasti lagi di %s \n Koordinatenya : %s - %s\n Apa itu Gempa Ada di link dibawah ini"+
				"yaa mazzeh %s\n", location,
			...
			randm := []string{
				"Konnichiwa " + msg.Alias_name + "\n Hilal lagi gaadaa \n aku Chiwa Bot salam kenall yaaaa, Sarangheo <3 \n Apa itu Gempa bumi ada di link berikut ini ya mazzeh...\n" + link,
				"ihh jangan SPAM DOng berisik tau aku lagi dijalan nih",
				"Yameteh yoo",
				"Ihhh kamu cantik banget",
				"Jangan jadi anak senja ya",
			}
			dt := &wa.TextMessage{
				To:       msg.Phone_number,
				IsGroup:  false,
